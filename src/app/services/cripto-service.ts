...
export class CriptoService {
  ...
  // Inyecta el HttpClient y guárdalo en una variable llamada 'http'
  http = inject(HttpClient);
  // Angular busca la etiqueta MONEDA_BASE en app.config.ts
  monedaGlobal = inject(MONEDA_BASE);
  // Borramos los datos falsos. Ahora solo devolvemos la petición al servidor.
  obtenerMonedasDeVerdad() {
    const urlAPI = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${this.monedaGlobal}&order=market_cap_desc&per_page=5&page=1`;
